// SETUP CONSTANTS FOR FACE JUDGEMENT TASK //
...
const pracfreq = .5; // prevalence of middle-eastern faces in the practice phase
...
const choicekeys = ['l', 'a'] // keys to make judgements ([0] = middle-eastern; [1] not middle-eastern)
...
for (i in _.range(10, 310, 10)) {
  if (_.range(10, 310, 10)[i] < 100) {
   images.push('stim/H0'+_.range(10, 310, 10)[i]+'.jpg')//middle-eastern
  } else {
...
function stimRandomizer(block,freq,numtrials) {
  ...
  // signals = _.sample(middle-eastern, Math.round(numtrials*freq));
  // noise = _.sample(caucasian, Math.round(numtrials*(1-freq)));
